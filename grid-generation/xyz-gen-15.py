...
def jacobi(R_min,R_max,R_np,r_min,r_max,r_np,theta_np,phi_np):
    R_doamin=np.linspace(R_min, R_max, num=R_np, endpoint=False, retstep=True)[0][0:15]
...
savez_compressed("jacobi_coordinates_and_weights_15",jacobi_cordinates=jacobi_cord, jacobi_weights=jacobi_weights,R_space=R_space, r_space=r_space, legendre_weights=legendre_weights, chebyshev_weights=chebyshev_weights)
...
rand=np.linspace(0, 36495360, num=7920, endpoint=False, retstep=True,dtype=int)[0].reshape((792,10))
np.save('rand_ind_15', rand)
