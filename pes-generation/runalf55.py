...
import sys
start=50 #(included)
stop=55  #(not included)
diff=stop-start #number of R values
previous_tot_config=int(36495360+7*(5*32*16.5*24*24*8))
...
previous_no_files=int(792+7*(264)) #number of files already generated
...
def generate_v(jacobi_cord,rand, file_indices):
    ...
    for j in file_indices:
        ...
        if file_exists:
            ...
            np_array=np.load(f'/gpfs/home/maaibrahim/AlF_dimer/AlF-draft-2/v_npz/{j}_v.npz')
            ...
            if len(v_exists) == 46080:
                ...
                for i, i_sys in enumerate(read('/gpfs/scratch/maaibrahim/xyz-2/'+str(j)+'.xyz', index=':', format='extxyz',parallel=False)):
                    ...
                    if int(ind[i]) in config_no_exists:
                        ...
                        print(j,'adding a row to an exisited file')
