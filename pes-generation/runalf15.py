...
jacobi_cord = np.load("jacobi_coordinates_and_weights_15.npz")['jacobi_cordinates']
jacobi_weights = np.load("jacobi_coordinates_and_weights_15.npz")['jacobi_weights']
rand = np.load("/gpfs/home/maaibrahim/AlF_dimer/AlF-draft-2/rand_ind_15.npy")
print(rand)
...
def generate_v(jacobi_cord,rand, file_indices):
    ...
    for j in file_indices:
        #print(f'Process {rank} is handling file index {j}')
        #sys.stdout.flush()
        ...
        R, r1, r2, theta1, theta2, phi = jacobi_cord[np.array(ind), 0], jacobi_cord[np.array(ind), 1], jacobi_cord[np.array(ind), 2], jacobi_cord[np.array(ind), 3], jacobi_cord[np.array(ind), 4], jacobi_cord[np.array(ind), 5]
...
file_indices_per_rank = np.arange(0,792)[rank::size]
