...
import sys
start=40 #(included)
stop=45  #(not included)
diff=stop-start #number of R values
previous_tot_config=int(36495360+5*(5*32*16.5*24*24*8))
...
previous_no_files=int(792+5*(264)) #number of files already generated
...
file_indices_per_rank = np.arange(previous_no_files,previous_no_files+132)[rank::size]
for element in [2232,2136,2187,2176,2173,2174,2155,2133,2159]:
    np.insert(file_indices_per_rank, 0, element)
